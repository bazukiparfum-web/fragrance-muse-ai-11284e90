// Shared safety logic (edge). Mirrored in src/components/zuki/safety.ts.
...
// Output guardrails — detect leaked system prompt or unsafe model output
const UNSAFE_OUTPUT = [
  /\[\[ZUKI_REFUSE\]\]/,
  /\b(nigger|faggot|retard|chink|spic|kike)\b/i,
  /you are zuki,? bazuki fragrance's/i, // leaked system prompt
  /SCENT KNOWLEDGE:|PERSONALITY:|WHAT YOU CANNOT DO:/, // headers from system prompt
];
...
export function classifyOutput(text: string): { blocked: boolean; reason: BlockReason } | null {
  if (UNSAFE_OUTPUT.some((r) => r.test(text))) {
    return { blocked: true, reason: /\[\[ZUKI_REFUSE\]\]/.test(text) ? "offtopic" : "unsafe" };
  }
  return null;
}
